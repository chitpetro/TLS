...
namespace GUI
{
    public partial class f_hd_khach : Form
    {
        ...
        public f_hd_khach()
        {
            ...
            var quyen2 = db.PhanQuyen2s.FirstOrDefault(p => p.TaiKhoan == Biencucbo.phongban && p.ChucNang == "HoaDonKhach_BaoCo");
            btnbaoco.Enabled = (bool)quyen2.Xem;
        }
        public void load()
        {
            var check = (from a in db.hoadons where a.link == Biencucbo.thanhtoan && a.dv == "1" select a);
            if (check.Count() == 0)
            {
                ...
                for (int i = 0; i < gridView16.DataRowCount; i++)
                {
                    ...
                    gridView1.SetFocusedRowCellValue("idsanpham", gridView16.GetRowCellValue(i, "idsanpham"));
                    gridView1.SetFocusedRowCellValue("dongia", gridView16.GetRowCellValue(i, "dongia"));
                    gridView1.SetFocusedRowCellValue("chietkhau", gridView16.GetRowCellValue(i, "chietkhau"));
                    gridView1.SetFocusedRowCellValue("thue", gridView16.GetRowCellValue(i, "thue"));
                    gridView1.SetFocusedRowCellValue("tygia", 1);
                    gridView1.SetFocusedRowCellValue("loaithue", gridView16.GetRowCellValue(i, "loaithue"));
            ...
            else
            {
                ...
                try
                {
                    var check2 = (from a in db.hoadons where a.dv == "1" select a).FirstOrDefault(t => t.link == Biencucbo.thanhtoan);
                ...
                else
                {
                    if (Biencucbo.hdhd == 0)
                    {
                        ...
                        try
                        {
                            ...
                            hd.moihd2(txtid.Text, txtngaylap.DateTime, txtiddt.Text, txtidnv.Text, txtdv.Text, txtghichu.Text, Convert.ToInt32(txt1.Text), "Bán hàng", txttiente.Text, double.Parse(txttygia.Text), txtbanhang.Text, "1", txtvat.Text);
                            for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                            {
                    ...
                    else
                    {
                        try
                        {
                            hd.suahd(txtid.Text, DateTime.Parse(txtngaylap.Text), txtiddt.Text, txtghichu.Text, int.Parse(txt1.Text), "Bán hàng", txttiente.Text, double.Parse(txttygia.Text), txtvat.Text);
                            //sua ct
                            LuuPhieu();
                            //định khoản
                            tk.xoa(txtid.Text);
            ...
            try
            {
                ...
                for (int i = 0; i <= gridView1.RowCount - 1; i++)
                {
                    if (checkmasp == 1)
                    {
                        try
                        {
                            string b = (gridView1.GetRowCellValue(i, "idsanpham").ToString());
                            var lst = (from a in db.dmtks where a.sanpham == true && a.active == true && a.tkme == "156" select a).FirstOrDefault(t => t.idsp == b);
                            if (lst != null)
                                checkmasp = 1;
                        }
                        catch
        ...
        private void f_hd_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Hoá Đơn Bán Hàng").ToString();
