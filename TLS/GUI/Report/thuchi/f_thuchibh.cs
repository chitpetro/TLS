...
{
    public partial class f_thuchibh : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void f_chitietnhapkho_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "SỔ QUỸ TIỀN MẶT").ToString();
            ...
            try
            {
                ...
                {
                    var lst = from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_thuchibh" select a;
                    nhan.DataSource = lst;
                    var list = (from a in db.donvis
                                ...
                                select new
                                {
                                    id = a.id,
                                    name = a.tendonvi,
                                    key = a.id + danhmuc.Text + Biencucbo.idnv + "f_thuchibh",
                                    MaTim = LayMaTim(a)
                                });
        ...
        private void danhmuc_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (danhmuc.Text == "Tiền Tệ")
            {
                try
                {
                    var list = (from a in db.tientebhs
                                select new
                                {
                                    id = a.tiente,
                                    name = a.tygia,
                                    key = a.tiente + danhmuc.Text + Biencucbo.idnv + "f_thuchibh",
                                });
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            if (danhmuc.Text == "Tiền Tệ")
            {
                try
                {
                    ...
                    db.dk_rps.InsertOnSubmit(dk);
                    db.SubmitChanges();
                    var lst2 = from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_thuchibh" select a;
                    nhan.DataSource = lst2;
                    var lst = (from a in db.tientebhs select new { id = a.tiente, name = a.tygia, key = a.tiente + danhmuc.Text + Biencucbo.idnv + "f_thuchibh" });
        ...
        private void gridView2_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (doubleclick2 == true)
            {
                ...
                {
                    dk_rp dk = new dk_rp();
                    var lst = (from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_thuchibh" select a).Single(t => t.key == gridView2.GetFocusedRowCellValue("key").ToString());
        ...
        private void simpleButton5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < gridView2.DataRowCount; i++)
                {
                    ...
                    if (gridView2.GetRowCellValue(i, "loai").ToString() == "Tiền Tệ")
                    {
                        check5++;
                ...
                if (tsntqd.IsOn == false)
                {
                    if (check5 == 0)
                    {
                        Lotus.MsgBox.ShowWarningDialog("Cần phải chọn 1 trường dữ liệu bắt buộc: Tiền Tệ");
                        return;
                    }
                }
                ...
                if (tsntqd.IsOn)
                {
                    ...
                    //            var tk2 = (from a in db.tientebhs
                    ...
                    var lst = (from a in db.nopquys
                               where a.ngaynop < tungay.DateTime
                               select new
                               {
                                   PS = a.thanhtien,
                                   iddv = a.dvnop,
                    ...
                    var lst3 = (from a in lst
                                ...
                                where b.user == Biencucbo.idnv && b.loai == "Đơn vị - ຫົວໜ່ວຍ" && b.form == "f_thuchibh"
                                select a.PS).Sum();
                    lst = (from a in db.thutienbanhangs
                           where a.ngaytt < tungay.DateTime
                           select new
                           {
                               PS = a.thanhtien,
                               iddv = a.dvtt,
                    ...
                    lst = (from a in db.nopquys

                           where a.ngaynop <= denngay.DateTime
                           orderby a.ngaynop ascending,
                            a.id descending
                           select new
                           {
                               PS = a.thanhtien,
                    ...
                    lst = (from a in db.thutienbanhangs
                           ...
                           where a.ngaytt <= denngay.DateTime
                           select new
                           {
                               PS = a.thanhtien,
                               iddv = a.dvtt,
                    ...
                    var lst2 = (from a in db.nopquys

                                where a.ngaynop >= tungay.DateTime && a.ngaynop <= denngay.DateTime
                                select new
                                {
                                    idthu = "",
                                    idchi = a.id,
                                    ...
                                    a.tiente,
                                    sotien = a.thanhtien,
                                    thanhtien = a.thanhtien - a.thanhtien,
                                }).Concat(from a in db.thutienbanhangs

                                          where a.ngaytt >= tungay.DateTime && a.ngaytt <= denngay.DateTime
                                          select new
                                          {
                                              idthu = a.id,
                                              idchi = "",
                    ...
                    for (int i = 0; i <= gridView2.DataRowCount - 1; i++)
                    {
                        if (gridView2.GetRowCellValue(i, "loai").ToString() == "Tiền tệ")
                        {
                            var abc = (from a in db.tientebhs select a).Single(t => t.tiente == gridView2.GetRowCellValue(i, "id").ToString());
                           
                        }
                    }
                    Biencucbo.tiente = "KIP";
                    Biencucbo.nt = 1;
                    Biencucbo.title = "SỔ QUỸ TIỀN MẶT";
                    r_sothuchibh xtra = new r_sothuchibh();
                    xtra.DataSource = lst7;
                    ReportPrintTool rpt = new ReportPrintTool(xtra);
                    rpt.PreviewForm.MdiParent = Application.OpenForms[0];
                    rpt.PreviewForm.Text = "BÁO CÁO SỔ QUỸ TIỀN MẶT BÁN HÀNG";
                    rpt.ShowPreview();
                }
                else
                {
                    var tk1 = from b in db.tientebhs select b;
                    for (int i = 0; i < gridView2.DataRowCount; i++)
                    {
                        if (gridView2.GetRowCellValue(i, "loai").ToString() == "Tiền Tệ")
                        {
                            string ma = gridView2.GetRowCellValue(i, "id").ToString();
                            tk1 = (from a in db.tientebhs
                                   where a.tiente == ma
                                   select a
                                  );
                    ...
                    var tk = tk1;
                    var lst = (from a in db.nopquys
                               join b in tk on a.tiente equals b.tiente
                               where a.ngaynop < tungay.DateTime
                               select new
                               {
                                   PS_nt = a.nguyente,
                    ...
                    var lst2 = (from a in db.nopquys
                                ...
                                select new
                                {
                                    ...
                                    a.tiente,
                                    sotien = a.nguyente,
                                    thanhtien = a.nguyente - a.nguyente,
                                }).Concat(from a in db.thutienbanhangs
                                          join b in tk on a.tiente equals b.tiente
                                          where a.ngaytt >= tungay.DateTime && a.ngaytt <= denngay.DateTime
                                          select new
                                          {
                                              ...
                                              diengiai = a.diengiai,
                                              a.tiente,
                                              sotien = a.nguyente - a.nguyente,
                                              thanhtien = a.nguyente,
                                          }).OrderBy(t => t.ngay);
                    Biencucbo.tondau2 = Biencucbo.tondau;
                    key = 1;
                    var lst7 = (from a in lst2
                    ...
                    Biencucbo.nt = 0;
                    Biencucbo.title = "SỔ QUỸ TIỀN MẶT BÁN HÀNG";
                    r_sothuchibh xtra = new r_sothuchibh();
                    xtra.DataSource = lst7;
