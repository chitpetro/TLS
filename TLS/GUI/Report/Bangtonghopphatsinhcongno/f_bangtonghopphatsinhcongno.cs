...
{
    public partial class f_bangtonghopphatsinhcongno : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        KetNoiDBDataContext db = new KetNoiDBDataContext();
        Boolean doubleclick1 = false;
        Boolean doubleclick2 = false;
        public f_bangtonghopphatsinhcongno()
        {
            InitializeComponent();
        ...
        private void f_chitietnhapkho_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Bảng Tổng Hợp Phát Sinh Công Nợ").ToString();
            ...
            try
            {
                var lst = from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_bangtonghopphatsinhcongno" select a;
                nhan.DataSource = lst;
                if (danhmuc.Text == "Đơn vị - ຫົວໜ່ວຍ")
                {
                    var list = (from a in db.donvis
                                select new
                                {
                                    id = a.id,
                                    name = a.tendonvi,
                                    key = a.id + danhmuc.Text + Biencucbo.idnv + "f_bangtonghopphatsinhcongno",
                                    MaTim = LayMaTim(a)
                                });
        ...
        private void danhmuc_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (danhmuc.Text == "Loại thu - ປະເພດ")
            {
                try
                {
                    var lst = (from a in db.dmloaithus select new { id = a.danhmuc, name = a.danhmuc_l == null ? "---" : a.danhmuc_l, key = a.danhmuc + Biencucbo.idnv + "f_bangtonghopphatsinhcongno" });
            ...
            else if (danhmuc.Text == "Mục Chi Phí - ບັນຊີລາຍຈ່າຍ")
            {
                try
                {
                    var lst = (from a in db.muccps select new { id = a.id, name = a.muccp1, key = a.id + danhmuc.Text + Biencucbo.idnv + "f_bangtonghopphatsinhcongno" });
                    nguon.DataSource = lst;
                    for (int i = gridView1.RowCount - 1; i >= 0; i--)
                    {
                        for (int j = 0; j < gridView2.DataRowCount; j++)
            ...
            else if (danhmuc.Text == "Đối Tượng - ເປົ້້າໝາຍ")
            {
                try
                {
                    var lst = (from a in db.doituongs select new { id = a.id, name = a.ten, key = a.id + danhmuc.Text + Biencucbo.idnv + "f_bangtonghopphatsinhcongno" });
                    nguon.DataSource = lst;
                    for (int i = gridView1.RowCount - 1; i >= 0; i--)
                    {
                        for (int j = 0; j < gridView2.DataRowCount; j++)
            ...
            else if (danhmuc.Text == "Công Việc - ໜ້າວຽກ")
            {
                try
                {
                    var lst = (from a in db.congviecs select new { id = a.id, name = a.tencongviec, key = a.id + danhmuc.Text + Biencucbo.idnv + "f_bangtonghopphatsinhcongno" });
                    nguon.DataSource = lst;
                    for (int i = gridView1.RowCount - 1; i >= 0; i--)
                    {
                        for (int j = 0; j < gridView2.DataRowCount; j++)
            ...
            else if (danhmuc.Text == "Tài Khoản")
            {
                try
                {
                    var lst = (from a in db.dmtks where a.active == true select new { id = a.matk, name = a.tentk, key = a.matk + danhmuc.Text + Biencucbo.idnv + "f_bangtonghopphatsinhcongno" });
                    nguon.DataSource = lst;
                    for (int i = gridView1.RowCount - 1; i >= 0; i--)
                    {
                        for (int j = 0; j < gridView2.DataRowCount; j++)
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dk.form = "f_bangtonghopphatsinhcongno";
        ...
        private void gridView2_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (doubleclick2 == true)
            {
                ...
                {
                    dk_rp dk = new dk_rp();
                    var lst = (from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_bangtonghopphatsinhcongno" select a).Single(t => t.key == gridView2.GetFocusedRowCellValue("key").ToString());
                    db.dk_rps.DeleteOnSubmit(lst);
                    db.SubmitChanges();
                    var lst2 = from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_bangtonghopphatsinhcongno" select a;
        ...
        private void simpleButton5_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var lst = (from a in db.dk_rps where a.user == Biencucbo.idnv && a.form == "f_bangtonghopphatsinhcongno" && a.loai == "Đơn vị - ຫົວໜ່ວຍ" select a);
                ...
                var lst2a = from a in lst2
                            join b in db.doituongs on a.iddt equals b.id
                            select new
                            {
                                a.taikhoan,
                                a.iddt,
                ...
                var ltdau = from a in lst3
                            select new
                            {
                                ...
                                a.iddt,
                                a.tendt,
                                psno = a.psno - a.psco > 0 ? a.psno - a.psco : a.psno - a.psno,
                                psco = a.psno - a.psco < 0 ? (a.psno - a.psco) * (-1) : a.psno - a.psno
                            };
                lst2 = (from a in lst1
                        join b in tk on a.tk_no equals b.matk
                ...
                var lttong = (from a in ltcuoi
                              join b in ltdau on new { a.taikhoan, a.iddt } equals new { b.taikhoan, b.iddt } into x
                              join c in ltgiua on new { a.taikhoan, a.iddt } equals new { c.taikhoan, c.iddt } into y
                              from giua in y.DefaultIfEmpty()
                              from dau in x.DefaultIfEmpty()
                              select new
                              {
                                  matk = a.taikhoan,
                                  a.tentk,
                                  ...
                                  psno_cuoi = a.psno,
                                  psco_cuoi = a.psco,
                                  psno_giua = giua.psno == null ? 0 : giua.psno,
                                  psco_giua = giua.psco == null ? 0 : giua.psco,
                                  psno_dau = dau.psno == null ? 0 : dau.psno,
                                  psco_dau = dau.psco == null ? 0 : dau.psco,
                              });
                r_bangtonghopphatsinhcongno xtra = new r_bangtonghopphatsinhcongno();
                xtra.DataSource = lttong;
                ReportPrintTool rpt = new ReportPrintTool(xtra);
                rpt.PreviewForm.MdiParent = Application.OpenForms[0];
                rpt.PreviewForm.Text = "Báo Cáo Bảng Tổng Hợp Phát Sinh Công Nợ";
