...
namespace GUI
{
    public partial class f_pmuahang : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        KetNoiDBDataContext db = new KetNoiDBDataContext();
        t_pmua pnct = new t_pmua();
        ...
        {
            InitializeComponent();
            //btnmasp1.DataSource = new DAL.KetNoiDBDataContext().sanphams;
            var lst = (from a in db.sanphams select new { id = a.id, tensp = a.tensp, dvt = a.dvt });
            if (lst == null) return;
            ...
            //lay quyen
            var quyen1 = db.PhanQuyen2s.FirstOrDefault(p => p.TaiKhoan == Biencucbo.phongban && p.ChucNang == "PhieuMuaHang_TaoHoaDonBanHang");
            btnhoadon.Enabled = (bool)quyen1.Xem;
            
            var quyen2 = db.PhanQuyen2s.FirstOrDefault(p => p.TaiKhoan == Biencucbo.phongban && p.ChucNang == "PhieuMuaHang_PhieuXuatKho");
        ...
        public void load()
        {
            ...
            try
            {
                var lst = (from a in db.pmuahangs where a.iddv == Biencucbo.donvi select a.so).Max();
                var lst1 = (from b in db.pmuahangs where b.iddv == Biencucbo.donvi select b).FirstOrDefault(t => t.so == lst);
                if (lst1 == null) return;
                gcchitiet.DataSource = lst1.pmuahangcts;
                txtid.Text = lst1.id;
        ...
        private void f_pnhap_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Phiếu Mua Hàng").ToString();
        ...
        {
            db = new KetNoiDBDataContext();
            f_dspmuahang frm = new f_dspmuahang();
            frm.ShowDialog();
            if (Biencucbo.getID == 1)
            {
                ...
                try
                {
                    var lst = (from pn in db.pmuahangs select new { a = pn }).FirstOrDefault(x => x.a.id == Biencucbo.ma);
                    if (lst == null) return;
                    txtid.Text = lst.a.id;
                    ...
                    gcchitiet.DataSource = lst.a.pmuahangcts;
        ...
        private void btnnew_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            txtid.Text = "YYYYY";
            gcchitiet.DataSource = new DAL.KetNoiDBDataContext().View_pmuahangs;
            ...
            else
            {
                ...
                else
                {
                    if (Biencucbo.hdpm == 0)
                    {
                        ...
                        try
                        {
                            string check = "PM" + Biencucbo.donvi.Trim().ToString();
                            var lst1 = (from s in db.tudongs where s.maphieu == check select new { so = s.so }).ToList();
                            if (lst1.Count == 0)
                            {
                                int so;
                                so = 2;
                                td.themtudong(check, so,"PM","Phiếu Mua Hàng");
                            ...
                            pnct.moipm(txtid.Text, txtngaynhap.DateTime, txtiddt.Text, txtdv.Text, txtidnv.Text, txtghichu.Text, Convert.ToInt32(txt1.Text), txtloainhap.Text, txttiente.Text, float.Parse(txttygia.Text));
                            for (int i = 0; i <= gridView1.RowCount - 1; i++)
                            {
                                gridView1.SetRowCellValue(i, "idpmuahang", txtid.Text);
                                ...
                                //{
                                //    thue = float.Parse(gridView1.GetRowCellValue(i, "thue").ToString());
                                //    giavon = float.Parse(gridView1.GetRowCellValue(i, "nguyente").ToString()) - thue;
                                    //tk.moi(txtid.Text + i + "1", txtdv.Text, "PN", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), gridView1.GetRowCellValue(i, "idsanpham").ToString(), txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), "156", "331", giavon * float.Parse(txttygia.Text), txttiente.Text, float.Parse(txttygia.Text), giavon, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, txtloainhap.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "");
                                    //tk.moi(txtid.Text + i + "2", txtdv.Text, "PN", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), txtiddt.Text, txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), "133", "331", thue * float.Parse(txttygia.Text), txttiente.Text, float.Parse(txttygia.Text), thue, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, txtloainhap.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "", txtghichu.Text, lbltendt.Text,0.0);
                                    
                                //}
                                //else if (txtloainhap.Text == "Điều chuyển nội bộ")
                                //{
                                //    tk.moi(txtid.Text + i + "1", txtdv.Text, "PN", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), gridView1.GetRowCellValue(i, "idsanpham").ToString(), gridView1.GetRowCellValue(i, "idsanpham").ToString(), gridView1.GetRowCellValue(i, "diengiai").ToString(), "156", "156", giavon * float.Parse(txttygia.Text), txttiente.Text, float.Parse(txttygia.Text), giavon, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, txtloainhap.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "");
                                //}
                            }
                            //btn
                            btnmo.Enabled = true;
                    ...
                    else
                    {
                        try
                        {
                            pnct.suapm(txtid.Text, DateTime.Parse(txtngaynhap.Text), txtiddt.Text, txtghichu.Text, int.Parse(txt1.Text), txtloainhap.Text, txttiente.Text, float.Parse(txttygia.Text));
                            //sua ct
                            LuuPhieu();
                            tk.xoa(txtid.Text);
                            for (int i = 0; i <= gridView1.RowCount - 1; i++)
                            {
                                if (txtloainhap.Text == "Nhập mua")
                                {
                                    thue = float.Parse(gridView1.GetRowCellValue(i, "thue").ToString());
                                    giavon = float.Parse(gridView1.GetRowCellValue(i, "nguyente").ToString()) - thue;
                                    //tk.moi(txtid.Text + i + "1", txtdv.Text, "PN", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), gridView1.GetRowCellValue(i, "idsanpham").ToString(), txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), "156", "331", giavon * float.Parse(txttygia.Text), txttiente.Text, float.Parse(txttygia.Text), giavon, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, txtloainhap.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "", txtghichu.Text, lbltendt.Text);
                                    //tk.moi(txtid.Text + i + "2", txtdv.Text, "PN", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), txtiddt.Text, txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), "133", "331", thue * float.Parse(txttygia.Text), txttiente.Text, float.Parse(txttygia.Text), thue, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, txtloainhap.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "", txtghichu.Text, lbltendt.Text);
                                }
                                else if (txtloainhap.Text == "Điều chuyển nội bộ")
                                {
