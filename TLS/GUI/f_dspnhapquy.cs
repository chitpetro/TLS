...
namespace GUI
{
    public partial class f_dspnhapquy : Form
    {
        ...
        public void loaddata(DateTime tungay, DateTime denngay)
        {
            ...
            var lst = from a in db.thutienbanhangs
                      join d in db.donvis on a.dvtt equals d.id
                      where
                      a.ngaytt >= tungay && a.ngaytt <= denngay && a.loai == "Nhập Quỹ"
                      select new
                      {
                          id = a.id,
                          ngaythu = a.ngaytt,
        ...
        private void f_PN_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Danh Sách Phiếu Thu Tiền Nhập Quỹ").ToString();
