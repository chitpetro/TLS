...
namespace GUI
{
    ...
    public partial class f_hddv : Form
    {
        ...
        public f_hddv()
        {
            ...
            //lay quyen
            var quyen1 = db.PhanQuyen2s.FirstOrDefault(p => p.TaiKhoan == Biencucbo.phongban && p.ChucNang == "HDDV_ThanhToan");
        ...
        public void load()
        {
            ...
            try
            {
                var lst = (from a in db.hoadons where a.iddv == Biencucbo.donvi && a.dv == "5"  select a.so).Max();
                var lst1 = (from b in db.hoadons where b.iddv == Biencucbo.donvi && b.dv == "5"  select b).FirstOrDefault(t => t.so == lst);
                if (lst1 == null) return;
                    ...
                    if (Biencucbo.hdhddv == 0)
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i <= gridView1.DataRowCount - 1; i++)
                            {
                                ...
                                tk.moi(txtid.Text + i + "1", txtdv.Text, "HD", txtid.Text, txtngaylap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text), txtiddt.Text, txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), "1311", "5113", doanhthu * double.Parse(txttygia.Text), txttiente.Text, double.Parse(txttygia.Text), doanhthu, gridView1.GetRowCellValue(i, "idsanpham").ToString(), "", txtidnv.Text, "Bán Hàng" /*txtloaihd.Text*/, gridView1.GetRowCellValue(i, "idcv").ToString(), "", lbltendt.Text, txtghichu.Text,0.0);
        ...
        private void f_hd_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Hoá Đơn Dịch Vụ").ToString();
