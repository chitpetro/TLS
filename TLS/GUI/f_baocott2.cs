...
namespace GUI
{
    public partial class f_baocott2 : Form
    {
        ...
        public void load()
        {
            ...
            var check = (from a in db.pthus where a.link == Biencucbo.thanhtoan && a.dv == "BC" select a);
            if (check.Count() == 0)
            {
                Biencucbo.hdpt = 0;
                ...
                txtghichu.Text = "Thu tiền bán hàng - ຮັບເງິນຂາຍສິນຄ້າ";
                txtloaithu.Text = "Thu tiền bán hàng - ຮັບເງິນຂາຍສິນຄ້າ";
                txtsohd.Text = Biencucbo.thanhtoan;
                gridView1.SetFocusedRowCellValue("nguyente", float.Parse(lst.tongtien.ToString()));
                gridView1.SetFocusedRowCellValue("thanhtien", float.Parse(lst.tongtien.ToString()) * float.Parse(lst.tygia.ToString()));
            ...
            else
            {
                if (Biencucbo.hdpt == 0)
                {
                    ...
                    try
                    {
                        ...
                        pthu.moipthu(txtid.Text, txtngaynhap.DateTime, txtiddt.Text, txtdv.Text, txtidnv.Text, txtghichu.Text, Convert.ToInt32(txt1.Text), txtsohd.Text, txttiente.Text, float.Parse(txttygia.Text), txtloaithu.Text, txttk.Text,"BC");
                        for (int i = 0; i <= gridView1.RowCount - 1; i++)
                        {
                            gridView1.SetRowCellValue(i, "idthu", txtid.Text);
                            gridView1.SetRowCellValue(i, "id", txtid.Text + i);
                            pthu.moict(gridView1.GetRowCellValue(i, "diengiai").ToString(), gridView1.GetRowCellValue(i, "idcv").ToString(), gridView1.GetRowCellValue(i, "idmuccp").ToString(), float.Parse(gridView1.GetRowCellValue(i, "thanhtien").ToString()), gridView1.GetRowCellValue(i, "idthu").ToString(), gridView1.GetRowCellValue(i, "id").ToString(), gridView1.GetRowCellValue(i, "tiente").ToString(), float.Parse(gridView1.GetRowCellValue(i, "tygia").ToString()), float.Parse(gridView1.GetRowCellValue(i, "nguyente").ToString()), gridView1.GetRowCellValue(i, "tkco").ToString());
                            tk.moi(txtid.Text + i + "1", txtdv.Text, "BC", txtid.Text, txtngaynhap.DateTime, new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day), Convert.ToInt32(txt1.Text),Biencucbo.donvi, txtiddt.Text, gridView1.GetRowCellValue(i, "diengiai").ToString(), txttk.Text, gridView1.GetRowCellValue(i, "tkco").ToString(), float.Parse(gridView1.GetRowCellValue(i, "thanhtien").ToString()), txttiente.Text, float.Parse(txttygia.Text), float.Parse(gridView1.GetRowCellValue(i, "nguyente").ToString()), "", "", txtidnv.Text, txtloaithu.Text, gridView1.GetRowCellValue(i, "idcv").ToString(), "", lbltendt.Text, txtghichu.Text, 0.0);
                        }
                        //btn
                        btnmo.Enabled = true;
                        btnnew.Enabled = true;
                ...
                else
                {
                    try
                    {
                        pthu.suapt(txtid.Text, DateTime.Parse(txtngaynhap.Text), txtiddt.Text, txtghichu.Text, int.Parse(txt1.Text), txtsohd.Text, txttiente.Text, float.Parse(txttygia.Text), txtloaithu.Text, txttk.Text);
                        //sua ct
                        LuuPhieu();
                        tk.xoa(txtid.Text);
                        for (int i = 0; i <= gridView1.RowCount - 1; i++)
        ...
        private void f_pthu_Load(object sender, EventArgs e)
        {
            ...
            this.Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Báo Có TT 2").ToString();
        ...
        private void btnfirst_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                var lsta = (from pn in db.pthus where pn.iddv == txtdv.Text select pn).Max(x => x.so);
        ...
        private void btnlast_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                var lsta = (from pn in db.pthus where pn.iddv == txtdv.Text select pn).Min(x => x.so);
                var lst1 = (from pn in db.pthus where pn.iddv == txtdv.Text select pn).FirstOrDefault(x => x.so == lsta);
                if (lst1 == null) return;
