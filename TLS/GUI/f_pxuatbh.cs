...
namespace GUI
{
    public partial class f_pxuatbh : Form
    {
            ...
            else
            {
                try
                {
                    ...
                    var lstdudau = (from a in new DAL.KetNoiDBDataContext().sodubandaus
                                    where (a.matk == "1561" || a.matk == "1562") && a.idsp == gridView1.GetFocusedRowCellValue("idsanpham").ToString()
                                    select new
                                    {
                    ...
                    if (tmua.ToList().Count == 0)
                    {
                        ...
                        gtmua = 0 + double.Parse(lstdudau.Sum(t => t.psno).ToString());
                    }
                    else
                    {
                        mua = double.Parse(tmua.Sum(t => t.soluong).ToString()) + double.Parse(lstdudau.Sum(t => t.slno).ToString());
                        gtmua = double.Parse(tmua.Sum(t => t.thanhtien).ToString()) + double.Parse(lstdudau.Sum(t => t.psno).ToString());
                    }
                    ...
                    if (tban.ToList().Count == 0)
                    {
                        ...
                        gtban = 0 + +double.Parse(lstdudau.Sum(t => t.psco).ToString());
                    }
                    else
                    {
                        ban = double.Parse(tban.Sum(t => t.soluong).ToString()) + double.Parse(lstdudau.Sum(t => t.slco).ToString());
                        gtban = double.Parse(tban.Sum(t => t.thanhtien).ToString()) + double.Parse(lstdudau.Sum(t => t.psco).ToString());
                    }
                    ...
                    else
                    {
                        ...
                        gridView1.SetFocusedRowCellValue("dongia", giavon);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void txtngaylap_Leave(object sender, EventArgs e)
        {
            for (int j = 0; j < gridView1.DataRowCount; j++)
            {
                ...
                {
                    var lstdudau = (from a in new DAL.KetNoiDBDataContext().sodubandaus
                                    where (a.matk == "1561" || a.matk == "1562") && a.idsp == gridView1.GetRowCellValue(j, "idsanpham").ToString()
                                    select new
                                    {
                                        slno = a.psno > 0 ? a.soluong : 0,
                                        psno = a.psno == null ? 0 : a.psno,
